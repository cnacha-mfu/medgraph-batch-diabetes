...
docs = db.collection(u'summary_diabetes').where(u'status', u'==', 1).stream()
for doc in docs:
...
for dep_doc in docs:
    ...
    update_time, doc_ref = db.collection(u'summary_diabetes').add(summary_row)
...
for doc in docs:
    ...
    db.collection(u'prediction_diabetes').document(record_ref.id).set(prediction_data)
    
    #combine explaination summary with current one
    pos_dict, neg_dict  = combine_expln(pos_dict,neg_dict, expln)
    
    ## record summary
    summary_docs = db.collection(u'summary_diabetes').where(u'department', u'==', item['department']).order_by(u'update_date', direction=firestore.Query.DESCENDING).limit(1).stream()
...
for factor in pos_dict:
    ...
    db.collection(u'expln_summary_diabetes').document("pos"+factor).set(factor_item)

for factor in neg_dict:
    factor_item = {'name':factor,'value':neg_dict[factor],'type':-1}
    db.collection(u'expln_summary_diabetes').document("neg"+factor).set(factor_item)
